...
这篇文章想整理一个相对务实的视角：**大多数 Agent 项目不应该一开始就追求完全自治，而应该先用 workflow 做骨架，再在需要弹性的地方嵌入 agentic 节点。** 至于 master + sub Agent，或者说 Supervisor + Worker，更适合作为 workflow 中的一种组织方式，而不是替代 workflow 的全部。
...
这不是保守，而是工程上更容易走得远。Agent 项目最怕的不是不够智能，而是看起来很智能，但每一次出错都不知道为什么。
...
这也是为什么我倾向于把 LangGraph 看成Agent 系统的编排层，而不只是写 Agent 的框架。它真正重要的地方，不是让我们多写几个 Agent，而是让 Agent 的行为有结构。
...
如果任务路径很难预先定义，而且每一步都依赖上一步的观察结果，可以使用 Agent Loop。
...
这里的核心不是“让一个 Agent 一口气写完整篇文章”，而是把研究过程拆开。检索和写作可以是两个不同节点；分析和验证也应该分开。这样做出的文章不仅更稳，也更容易解释“为什么最后选择这个架构”。
...
Agent 项目很容易demo 很惊艳，维护很痛苦。原因之一是没有评估。
