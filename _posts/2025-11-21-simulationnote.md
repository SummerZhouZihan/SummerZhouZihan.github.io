...
$$ P(x \to y) = g(y \vert x) \cdot h(x,y) $$
其中：
- $ g(y \vert x) $是建议分布（Metropolis算法中要求它是**对称的**，即$ g(y \vert x) = g(x|y) $）；
- $ h(x,y) = \min\left\{1, \frac{f(y)}{f(x)}\right\} $是接受概率。

我们需要证明：$ f(x) \cdot h(x,y) = f(y) \cdot h(y,x) $（结合对称的建议分布$ g(y \vert x)=g(x \vert y) $，即可推出$ f(x)P(x\to y)=f(y)P(y\to x) $）。
...
无论$ f(x) $和$ f(y) $的大小关系如何，都能验证 $ f(x)h(x,y) = f(y)h(y,x) $。结合对称的建议分布$ g(y \vert x)=g(x \vert y) $，可进一步推出：

$$ f(x) P(x \to y) = f(y) P(y \to x) $$

这说明Metropolis算法构造的马尔可夫链满足细致平稳条件，因此其平稳分布就是目标分布$ f(x) $. 
#### 8.3.2 MCMC的其他性质
